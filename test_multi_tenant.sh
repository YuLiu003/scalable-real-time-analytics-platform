...
API_ENDPOINT="http://$MINIKUBE_IP:30085"  # Flask API service NodePort
...
RESPONSE=$(curl -v -s "$API_ENDPOINT/api/data?device_id=$DEVICE_T1" \
  -H "X-API-Key: test-key-1" \
  -w "\n%{http_code}")
...
RESPONSE=$(curl -v -s "$API_ENDPOINT/api/data?device_id=$DEVICE_T2" \
  -H "X-API-Key: test-key-2" \
  -w "\n%{http_code}")
