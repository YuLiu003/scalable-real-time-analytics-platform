...
# Get a tunneled URL from minikube for the service
echo -e "${BLUE}Setting up service tunnel...${NC}"
API_URL=$(minikube service flask-api-service -n analytics-platform --url)
if [ -z "$API_URL" ]; then
  echo -e "${RED}Failed to get service URL. Make sure minikube is running with tunnel support.${NC}"
  exit 1
fi

echo -e "${GREEN}Using API endpoint: $API_URL${NC}"
...
RESPONSE=$(curl -s "$API_URL/api/data?device_id=$DEVICE_T1" \
  -H "X-API-Key: test-key-1" \
  -w "\n%{http_code}")
...
RESPONSE=$(curl -s "$API_URL/api/data?device_id=$DEVICE_T2" \
  -H "X-API-Key: test-key-2" \
  -w "\n%{http_code}")
...
# Note: minikube service tunnel will remain open. Stop it manually if needed.
echo -e "${YELLOW}Note: Service tunnel might still be running. Close the terminal to stop it.${NC}"